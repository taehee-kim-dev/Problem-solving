...
class Node:
    def __init__(self, parent_node, char):
        ...
        # 이 노드를 기준으로 자식노드들이 각 깊이별로 몇개씩 있는지
...
class Trie:
    ...
    def insert_node(self, word):
        ...
        # 현재 추가하고있는 글자 이후로 남아있는 글자의 개수
        remaining_length = len(word)
        # 단어에서 한 글자씩 차례대로 꺼내서 추가
        for char in word:
            if remaining_length in current_node.number_of_remain_length_from_this_node:
                """
                현재 추가하고있는 글자 이후로 남아있는 글자의 개수가
                현재 사용중인 노드의 number_of_remain_length_from_this_node에 key로 존재하면
                해당 key값의 value를 1 증가
                """
                current_node.number_of_remain_length_from_this_node[remaining_length] += 1
            else:
                """
            ...
            if char not in current_node.child_char_nodes:
                # 현재 추가중인 글자의 노드가 현재 검사중인 노드의 자식노드에 없다면
                # 새로 만들어서 자식노드에 추가
                current_node.child_char_nodes[char] = Node(current_node, char)

            # 현재 노드를 현재 추가한 글자의 노드로 변환 (한 단계 깊이 내려감)
    ...
    def search(self, query):
        ...
        """
        순수 단어의 글자들을 모두 검색한 이후
        물음표 검사 차례
        """
        if number_of_question_mark in current_node.number_of_remain_length_from_this_node:
            """
            물음표의 개수가 현재 검사중인 노드의 number_of_remain_length_from_this_node의 key값으로 존재하면
